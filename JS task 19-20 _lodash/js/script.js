...
//SKILLS................................................
var skills = [];//Ищем и вытаскиваем значения skills
...
var allSkills = [];//из многомерного массива со скилами делаем простой массив
_.forEach(skills, function(value){
	  _.forEach(value, function(el){
	  el = el.toLowerCase();//все в нижний регистр для сортировки
	  allSkills.push(el);
		});
});

var uniqueSkills = _.uniq(allSkills);//убираем посторяющиеся значения
var sortedSkills = uniqueSkills.sort();//sort skills
//console.log('sortedSkills = ', sortedSkills);

// Names..................................
var newArr = [];//Ищем и вытаскиваем значения name, friends	=> newArr
_.forEach(arr, function(obj){ 
		var i = _.pick(obj, ['name', 'friends']);
		newArr.push(i);
});


var sortFriends = _.sortBy(newArr, function(o){return o.friends.length; })//Сортируем по количеству друзей

var sortNames = [];// Оставляем только объекты с именами после сортировки
_.forEach(sortFriends, function(obj){ 
		var name = _.pick(obj, ['name']);
		sortNames.push(name);
});

var sortNamesArr = [];//Переводим массив объектов в массив подмассивов
_.forEach(sortNames, function(obj){ 
		var name = _.toArray(obj);
		sortNamesArr.push(name);
});
var sortNamesArrFinal = _.flatten(sortNamesArr); //...и в один массив
//console.log('finalFinal = ', sortNamesArrFinal);


// FRIENDS................
var friendsArr = [];//Ищем и вытаскиваем значения friends	=> friendsArr
_.forEach(arr, function(obj){ 
		var i = _.pick(obj, ['friends']);
		friendsArr.push(i);
});


var NameId = [];//Вытаскиваем в массив все объекты с id, name друзей
_.forEach(friendsArr, function(o){
	_.forEach(o, function(ob){
		_.forEach(ob, function(obj){
				NameId.push(obj);
		});
	});
});

var friendName = [];//Убираем id
_.forEach(NameId, function(obj){ 
		var name = _.pick(obj, ['name']);
		friendName.push(name);
});


var friendNameArr = [];//Переводим массив объектов в массив подмассивов
_.forEach(friendName, function(obj){ 
		var name = _.toArray(obj);
		friendNameArr.push(name);
});

var friendNameArray = _.flatten(friendNameArr); //...и в один массив
var uniqueFriendNameArray = _.uniq(friendNameArray);//убираем посторяющиеся значения

//console.log('uniqueFriendNameArray = ', uniqueFriendNameArray);
